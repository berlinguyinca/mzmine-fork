...
public class MiniXImportModule implements MZmineProcessingModule, TaskListener {
	...
	public ExitCode runModule(@Nonnull ParameterSet parameters,
		...
		String studyID = parameters.getParameter(
				MiniXImportParameters.MINIX_ID_PARAMETER).getValue();

		String minixUrl = parameters.getParameter(
				MiniXImportParameters.MINIX_URL_PARAMETER).getValue();
		...
		try {
			...
			URL getSamplesURL = new URL(minixUrl
					+ "/rest/listSamplesForStudyId/" + studyID);
		...
		if (aquireFiles(tasks, rootNode, parameters))
			return ExitCode.ERROR;
	 ...
	 * @return
	 */
	private boolean aquireFiles(Collection<Task> tasks, JsonNode rootNode,
			ParameterSet parameterSet) {

		String rawdataUrl = parameterSet.getParameter(
				MiniXImportParameters.MINIX_RAWDATA_URL).getValue();
		...
		for (JsonNode node : rootNode) {
			...
			try {
				getDataURLCon = (HttpURLConnection) new URL(rawdataUrl
						+ "/data/" + sampleID).openConnection();
