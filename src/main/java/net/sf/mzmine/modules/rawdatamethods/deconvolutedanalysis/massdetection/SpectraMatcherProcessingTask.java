...
public class SpectraMatcherProcessingTask extends AbstractTask {
	...
	public SpectraMatcherProcessingTask(RawDataFile dataFile, SpectrumType ionizationType, List<MassCandidate> massCandidates) {
		this.dataFile = dataFile;
        this.ionizationType = ionizationType;
        this.adducts = SpectraMatcherParameters.ADDUCT_PARAMS[ionizationType.ordinal()];
        this.matchesThreshold = SpectraMatcherParameters.ADDUCT_MATCHES[ionizationType.ordinal()].getValue();
	...
	public void run() {
		...
		for (int scanNumber : dataFile.getScanNumbers(1)) {
            ...
            // IMPORTANT: We assume the truncating the double-valued ion masses will
            ...
            // Find candidate masses by iterating over all possible mass values up to m/z = 1000
			for (int i = 1; i <= 1000; i++) {
                ...
                if(adductMatches.size() > matchesThreshold) {
                    massCandidates.add(new MassCandidate(dataFile, scanNumber, spectrum.getRetentionTime(), i, ionizationType, adductMatches));
                    matchesFound++;
                }
			}
		...
		if (!isCanceled()) {
			...
			LOG.info("Finished spectra mass matching on " + dataFile +", found "+ matchesFound);
		}
	}
}
