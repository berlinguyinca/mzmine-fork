...
public class DeconvolutedSpectraFilterParameters extends SimpleParameterSet  {
    ...
    public static final DoubleParameter C13_ISOTOPE_CUT = new DoubleParameter("C13 Isotope Cut",
            "Remove all C13 isotope ions whose previous ion (one less m/z value) is the given percentage larger (default 50% larger).", NumberFormat.getPercentInstance(), 0.5, 0.0, 1.0);

    public static final IntegerParameter NOISE_THRESHOLD = new IntegerParameter(
            "Noise Threshold", "Remove all ions less than the given noise threshold level.", 100);

    public static final DoubleParameter BASE_PEAK_CUT = new DoubleParameter("Base Peak Percentage Cut",
            "Remove all ions less than the given percentage of the current spectra's base peak (default 1%).",
            NumberFormat.getPercentInstance(), 0.01, 0.0, 1.0);
    ...
    public DeconvolutedSpectraFilterParameters() {
        super(new Parameter[]{dataFiles, C13_ISOTOPE_CUT, NOISE_THRESHOLD, BASE_PEAK_CUT, SUFFIX, REMOVE_ORIGINAL});
    }
}