...
public class BaselineCorrectionTask extends AbstractTask {
	...
	public BaselineCorrectionTask(final RawDataFile dataFile,
		...
		// Get parameters.
		suffix = parameters.getParameter(BaselineCorrectionParameters.SUFFIX)
				.getValue();
		removeOriginal = parameters.getParameter(
				BaselineCorrectionParameters.REMOVE_ORIGINAL).getValue();
		smoothing = parameters.getParameter(
				BaselineCorrectionParameters.SMOOTHING).getValue();
		asymmetry = parameters.getParameter(
				BaselineCorrectionParameters.ASYMMETRY).getValue();
		binWidth = parameters.getParameter(
				BaselineCorrectionParameters.MZ_BIN_WIDTH).getValue();
		useBins = parameters.getParameter(
				BaselineCorrectionParameters.USE_MZ_BINS).getValue();
		msLevel = parameters
				.getParameter(BaselineCorrectionParameters.MS_LEVEL).getValue();
		chromatogramType = parameters.getParameter(
	 ...
	 *             if there are i/o problems.
	 */
	private void correctBasePeakBaselines(final RawDataFileWriter writer,
			final int level, final int numBins) throws IOException {
	 ...
	 *             if there are i/o problems.
	 */
	private void correctTICBaselines(final RawDataFileWriter writer,
			final int level, final int numBins) throws IOException {
	 ...
	 */
	private double[][] buildBasePeakChromatograms(final int level,
			final int numBins) {
	...
	/**
	 * Determine the baseline via Asymmetric Least Squares (in R's parametric
	 * time warping PTW package).
	 * 
	 * @param chromatogram
	 *            the chromatogram whose baseline is to be determined.
	 * @return the baseline.
	 */
	private double[] asymBaseline(final double[] chromatogram) {
		...
		synchronized (RUtilities.R_SEMAPHORE) {
			...
			try {
				...
				throw new IllegalStateException(
						"R error during baseline correction", t);
			}
		}
		return baseline;
	}
